...
function InitTable(pageIndex, pageSize) {
	$.ajax({
				url : "${ctx}/goodsFront/ajax_listIndex.do?pageIndex=" + pageIndex
						+ "&pageSize=" + pageSize, //提交到一般处理程序请求数据            //提交两个参数：pageIndex(页面索引)，pageSize(显示条数)
				type : "POST",
				dataType : "json",
				success : function(data) {
...
$(function() {//Init
	$.ajax({
		url : "${ctx}/goodsFront/ajax_listIndex.do",
		type : "POST",
		dataType : "json",
		success : function(data) {
...
function findlike(){
	...
	$.ajax({
		type:"POST",
		url:"${ctx}/goodsFront/find.do?content="+content,
		success:function(data){
...
<div id="memberbox" class="memberbox">
	...
	<div class="w980">
		...
		您好，欢迎来到大咖网在线销售平台  <img src="${ctx}/static/shopping_files/images/m.gif" align="absmiddle"> <a href="#">商城手机版</a> 
		</li> 
...
<div class="item_header">
			<span style="font-size:18px;font-weight:bold;"><img src="${ctx}/static/shopping_files/images/tag.gif" align="absmiddle"> 动漫精选</span> 
			<div class="clr"></div> 
		</div>
				<div id="detail">
	...
	<div style="float:right;width:240px;padding-top:30px;">
		...
		<div style="border:1px solid #efefef;width:220px;background-color:white;">
				<div style="padding:10px;font-size:18px;font-weight:bold;text-align:center">点击排行</div>
				<div id="guess">
		...
		<div style="border:1px solid #efefef;width:240;padding:10px;">
				...
				<shiro:lacksRole name="普通用户">
						<a class="member_a"  style="padding-left:40px;" href="javascript:member_dialog('${ctx}/views/front/login.jsp',850,450,'');">有您的建议，动漫网才能更好</a>  
						</shiro:lacksRole>
							<shiro:hasRole name="普通用户">
							<a href="javascript:member_dialog('${ctx}/views/front/message.jsp',500,400,'');" style="padding-left:40px;">有您的建议，动漫网才能更好</a>
							</shiro:hasRole>
						</div>
				</div>
						</div>
