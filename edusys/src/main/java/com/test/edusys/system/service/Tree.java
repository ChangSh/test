		...
		for (Menu menu : Menus) {
			...
			if (menu.getPid() == null || menu.getPid() == 0) {
				...
				if (StringUtils.isNotEmpty(menu.getImagesrc())) {
					s = "<img src=\"/edusys/static/images/" + menu.getImagesrc() + "\">";
				}
				if (StringUtils.isNotEmpty(menu.getUrl())) {
					html.append("\r\n<li id='" + id + "'><a href=\"javascript:go('" + menu.getUrl() + "');\">" + s
							+ menu.getMenuname() + "</a>");
				} else {
					html.append("\r\n<li id='" + id + "'><a href=\"javascript:void(0)\">" + s + menu.getMenuname()
							+ "</a>");
				}
				build(menu);
				html.append("\r\n</li>");
	...
	private void build(Menu Menu) {
		...
		if (!children.isEmpty()) {
			...
			for (Menu child : children) {
				Long id = child.getId();
				html.append("\r\n<li id='" + id + "'><a href=\"javascript:go('" + child.getUrl() + "');\">"
						+ child.getMenuname() + "</a></li>");
				build(child);
			}
			html.append("\r\n</ul>");
		}
