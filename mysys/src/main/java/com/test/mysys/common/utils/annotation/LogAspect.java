package com.test.mysys.common.utils.annotation;
...
import com.test.mysys.system.service.LogService;
...
public class LogAspect {
	...
	// public void insertServiceCall(JoinPoint joinPoint, Object rtv) throws
	// Throwable{
	...
	private String getDescription(JoinPoint joinPoint) {
		...
		if (methodName.indexOf("insert") == 0) {
			...
			String sDescription = String.format("对%s信息执行了操作，记录为%s", record.getString("tablename"), name);
			return sDescription;
			...
			dao.execute(sql);
			String sDescription = String.format("对%s信息执行了操作，记录为%s", record.getString("tablename"),
			...
			String sDescription = String.format("对%s信息执行了操作，信息变更对比为%s", record.getString("tablename"), sRes);
			return sDescription;
		}
