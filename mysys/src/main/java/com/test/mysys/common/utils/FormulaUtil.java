...
public class FormulaUtil {
	 ...
	 * 构造方法，以要算的公式为参数
	 */
	public FormulaUtil(String calRule) {
		this.setFormulaUtil(calRule);
	}
	 ...
	 * 设置格式
	 */
	public void setFormulaUtil(String calRule) {
	...
	public double getResult() {
		...
		if (checkValid()) {
			for (int i = 0; i < leftBracket; i++) {
				...
				calRule = calRule.substring(0, iStart).trim() + value
	...
	public void FormulaStr(String calRule) {
		...
		if (checkValid()) {
			for (int i = 0; i < leftBracket; i++) {
				...
				calRule = calRule.substring(0, calRule.lastIndexOf("(")).trim() + value
						+ calRule.substring(calRule.indexOf(")") + 1, calRule.length()).trim();
	...
	/*
	 * public static void main(String[] args) { String s = "1.32/2.21";
	 * FormulaUtil test1 = new FormulaUtil(s);
