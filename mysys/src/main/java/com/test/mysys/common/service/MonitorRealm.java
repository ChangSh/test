...
public class MonitorRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		Role role = dao.fetch(Role.class, Cnd.where("id", "in",
				" SELECT roleid FROM t_system_user_role WHERE userid=" + principal.getId() + " "));
		if (role != null) {
			authorizes = dao.query(Authorize.class, Cnd.where("id", "in",
					" SELECT auth_id from t_system_role_authorize WHERE role_id =" + role.getId() + "  "));
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(new Principal(user), // 用户名
				user.getPassword(), // 密码
