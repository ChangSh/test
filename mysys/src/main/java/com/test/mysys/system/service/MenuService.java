...
public class MenuService extends BaseService {
	...
	public List<Menu> queryMenusByCurrentUserRole(String pid) {
		...
		String sSql = "SELECT a.* FROM t_system_user d,t_system_user_role c,t_system_role_menu b,t_system_menu a "
				+ " WHERE d.id = c.userid" + " AND a.id=b.menuid" + " AND c.roleid=b.roleid"
				+ " AND d.loginname=@loginname " + " AND a.visible=1 ";
	...
	public List<Menu> queryMenusByRole(String roleid) {
		Sql sql = Sqls
				.create("SELECT a.*,b.id as bid FROM t_system_menu a LEFT JOIN t_system_role_menu b ON b.menuid =a.id AND b.roleid="
						+ roleid + " order by sx");
