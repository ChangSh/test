...
public class ShiroFilerChainManager {
  ...
  public void initFilterChains(List<Resource> resources) {
    ...
    for (Resource resource : resources) {
      // 注册perms filter
      if (!StringUtils.isEmpty(resource.getAuthUrl()) && !StringUtils.isEmpty(resource.getUrl())) {
        filterChainManager.addToChain("/" + resource.getUrl(), "perms", resource.getAuthUrl());
      }
    }
    // 按照匹配顺序最后加
  ...
  private DefaultFilterChainManager getFilterChainManager() {
    ...
    /**
     * 获取filterchain解析器，PathMatchingFilterChainResolver内部通过FilterChainManager维护着拦截器链
     */
