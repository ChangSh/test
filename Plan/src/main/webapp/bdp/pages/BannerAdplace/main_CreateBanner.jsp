...
<!--选择创意界面  -->
  ...
  object_operate.show_add_modal=function(date){
	...
	//	var bannertype=$("#"+menuObject.parentNode.id+" input[name='ori']").val();
	 	var orderid=$("#OrderCode").val();
		ajaxHander("../Banner/getBannerbyOrderId.do",{orderid:orderid},"POST");
	}
	object_operate.add=function(date){
        ...
        function addEvent(date){
          ...
          var    opertionn = {
                  ...
                  afterShow: function(){
                  	...
                  	if(this.bgColor=="#ff33cc" ||　this.bgColor=="#00FF00"){
                  		$("#添加创意").show();
                  		$("#删除创意").hide();
                  	}else{
                  		$("#添加创意").hide();
							                          ...
							                          $.ajax({
							                			 ...
							                			 success:function(datajson){
							                				...
							                				var row=(nodes[i].parentNode.rowIndex-dateclumn.rowIndex-2);
    							                      		 nodes[i].outerHTML="<td class='tdmenu' bgcolor='#00FF00'>"+datajson[0].allowance+"<input type='hidden' name='e["+Onum+"].adplaceid' value='"+adplaceid+"'/>"
    							                      		 +"<input type='hidden' name='e["+Onum+"].reservedate' value='"+reservedate+"'/><input type='hidden' name='e["+Onum+"].state' value='0'/></td>";       
    							                      	     Onum++; 
							                			 }
							                			 });
							                      }
            ...
            if(tempdata.length!=0){
	            ...
	            for(var i=0;i<num;i++){ 
	            		 $("#tbEnd"+date.c_year+date.c_month).before(' <tr id="first'+date.c_year+date.c_month+"-"+(number+4)+'">'+
	            				 '<input type="hidden" name="adpId" value="'+tempdata[i].adpId +'"/>'+
	            				 '<input type="hidden" name="ori" value="'+tempdata[i].originality +'"/>'+
        ...
        function ajaxHander(url,data,type,value){
     	   $.ajax({
     			 ...
     			 success:function(data){ //请求成功后处理函数。 
	     			...
	     			if(url.indexOf("getBannerbyOrderId.do")>0){
	 ...
	 //按日期取出
  	...
  	function changeState(obj,date){
    	...
    	var dateclumn=document.getElementById("dateclumn"+id);//解决不同浏览器的索引值不一样问题
       for(var i=6;i<len;i++){
    		if(nodes[i].bgColor=="#ff33cc"|| nodes[i].bgColor=="#00FF00"){
            	var clumn=dateclumn.childNodes[nodes[i].cellIndex].innerText;
            	var orderid=$("#OrderCode").val();
                var adpid=$("#"+obj.parentNode.id+" input[name='adpId']").val();
            	var reservedate=id.substring(0,4)+"-"+id.substring(4,id.length)+"-"+clumn;
            	var reserveHour=$("#"+obj.parentNode.id+" input[name='reserveHour']").val();
         	    var datastr={orderId:orderid,adpId:"6",date:""+reservedate+"",reserveHour:reserveHour};
               $.ajax({
     			 async : false,
     			 type: "POST",
     	         data:datastr,
     			 ...
     			 success:function(datajson){
           		...
           		+"<input type='hidden' name='e["+Onum+"].bannerid' value='"+bannerid +"'/><input type='hidden' name='e["+Onum+"].reservedate' value='"+reservedate+"'/>"+
           		"<input type='hidden' name='e["+Onum+"].state' value='1'/></td>";       
           	    Onum++; 
     			 }
     			 });
