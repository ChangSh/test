...
TabPanel.prototype = {
  ...
  init : function(){
    ...
    $(".tabpanel_content").append('<div id="progressBar" class="progressBar" style="display: none;"><img src="resources/bdp/images/loadingbar.gif" /></div>');
  ...
  show : function(position, notExecuteMoveSee){
    ...
    if(this.scrollFinish)
    {
      ...
      initHeight(getTabHeight(this.tabs[position].id)); //设置页面高度
    }
  },
  /**
  ...
  setClosable : function(position,closable){
    ...
    if(position < this.tabs.length){
      ...
      if(closable){
        ...
        this.tabs[position].closer.addClass('icon-remove');
