    ...
    $.flexbox = function(div, o) {
		...
		function pinyin(chinese) {
		    ...
		    for (var i=0;i<l;i++) {
		        ...
		        if(reg.test(val)) {
		        	quanpin += val;
		        	jianpin += val;
		        } else if ((p = findpy(val)) != null) {
		        	quanpin += p;
		        	if(p.length > 0){
		        		jianpin += p.substr(0,1);
		        	}
		        }
		    }
		    var pylist = [];
		    pylist.push(quanpin.replace(/ /g,''));
		    pylist.push(jianpin.replace(/ /g,''));
		    return pylist;
		}
		function findpy(chinese){
		    for (var py in PY){
		        if (PY[py].indexOf(chinese)!=-1) {
		            return py;
		        }
		    }
		    return null;
		}
		function unparsepy(str) {
		    return escape(str).replace(/%u/gi, '\\u');
		}
		
		function initpy(data){
			if (typeof (data) === 'object') {
				for (var i=0; i < data[o.resultsProperty].length; i++) {
					var pylist = pinyin(data[o.resultsProperty][i][o.displayValue].toLowerCase());
					data[o.resultsProperty][i]["quanpin"] = pylist[0];
					data[o.resultsProperty][i]["jianpin"] = pylist[1];
				}
			}
		}
		...
		function filter(data, params) {
			...
			for (var i=0; i < data[o.resultsProperty].length; i++) {
				...
				if(o.pinyinSearch){
					if(indexOfMatch == -1){
						indexOfMatch = data[o.resultsProperty][i]["quanpin"].indexOf(params.q.toLowerCase());
						if(indexOfMatch == -1){
							indexOfMatch = data[o.resultsProperty][i]["jianpin"].indexOf(params.q.toLowerCase());
						}
					}
				}
        ...
        function outputPagingLinks(totalPages, currentPage, totalResults) {
            ...
            var first = '&lt;&lt;',
            //prev = '&lt;',
            prev = "<span class='previmg'>&nbsp;</span><span class='prevtext'>上一页</span>";
            //next = '&gt;',
            next = "<span class='nextimg'>&nbsp;</span><span class='nexttext'>下一页</span>";
            last = '&gt;&gt;',
            more = '...';
