...
#${model}_view_form .col-sm-9 label {
	font-weight: normal;
}

#${model}_form .col-sm-9 label {
	font-weight: normal;
}
</style>
...
<body>
	<div class="container">
		<div class="row">
				...
				<div class="panel-heading">
					<h4 class="panel-title">
						<a data-toggle="collapse"
							...
							href="#search${model}"> 搜索 </a>
					</h4>
				</div>
				<div id="search${model}" class="panel-collapse collapse">
					<div class="panel-body">
						<form id="${model}_searchForm">
							<div class="form-group input-group">
								<span class="input-group-addon">条件</span> <input type="text"
									class="form-control" style="width: 200px;" name="" />
							</div>
							<div style="float: right;">
								<input type="button" value="搜索" id="${model}_search"
									class="btn btn-Default" /> <input type="button" value="清空"
									id="${model}_searchReset" class="btn btn-Default" />
							</div>
						</form>
					</div>
				</div>
		 ...
		 <!--功能菜单 -->
			<div id="${model}_toolbar">			
				<input type="button" class="btn btn-primary" value="新增"
					id="${model}_add">
			</div>
			<table id="${model}_table">
			</table>
		</div>
		<!--第一层模态框  -->
		<div class="modal fade" id="${model}_Modal" tabindex="-1" role="dialog"
			aria-labelledby="${model}_ModalTitle" aria-hidden="true">
			<div class="modal-dialog" style="width: 600x;">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"
							aria-hidden="true">&times;</button>
						<h4 class="modal-title" id="${model}_ModalTitle"></h4>
					</div>
					<div class="modal-body">
						<form id="${model}_form" class="form-horizontal">
						   ${pagea}																																									
						</form>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal"
							id="${model}_close">关闭</button>
						<button type="button" class="btn btn-primary" id="${model}_submit">提交</button>
					</div>
				</div>
			</div>
		</div>

		<div class="modal fade" id="${model}_view_Modal" tabindex="-1"
			role="dialog" aria-labelledby="${model}_view_ModalTitle"
			aria-hidden="true">
			<div class="modal-dialog" style="width: 580x;">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"
							aria-hidden="true">&times;</button>
						<h4 class="modal-title" id="${model}_view_ModalTitle">查看用户详情</h4>
					</div>
					<div class="modal-body">
						<form id="${model}_view_form" class="form-horizontal">
							${pagei}												
						</form>
					</div>
					<div class="modal-footer">
